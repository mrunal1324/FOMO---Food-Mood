...
async def main():
    ...
    print("Welcome to FeelFood - Your Mood-Based Food Recommender!")
    print("Commands:")
    print("- 'location <city>' to set your location")
    print("- 'weather on/off' to toggle weather-based recommendations")
    print("- 'quit' to exit")
    print("\nTell me how you're feeling today...")
    
    while True:
        user_input = input("\nHow are you feeling? (or enter a command): ")
        ...
        if user_input.lower() == 'quit':
            ...
            continue
        elif user_input.lower() == 'weather on':
            recommender.toggle_weather()
            continue
        elif user_input.lower() == 'weather off':
            recommender.toggle_weather()
            continue
            
        mood, recommendations, weather = await recommender.get_recommendation(user_input)
        ...
        if feedback.lower() == 'yes':
            food_choice = input("Which food would you like to try? (enter number): ")
            try:
                food_choice = int(food_choice) - 1
                if 0 <= food_choice < len(recommendations):
                    recommender.update_user_preferences(mood, recommendations[food_choice])
                    print("Great! I'll remember your preference for next time!")
            except ValueError:
                print("Invalid input. No preferences were saved.")
